...
public partial class ReceptionistmakeAppointment : System.Web.UI.Page
{
    ...
    {
        string SelectSQL;
        SelectSQL = "select pf_name, pn_name, patient_id, dob FROM patient_info ";
        SelectSQL += "WHERE dob = :dob and upper(pf_name)=upper(:pf_name) and upper(pn_name)=upper(:pn_name) ";

        OracleConnection con = new OracleConnection(connectionString);
        OracleCommand selcmd = new OracleCommand(SelectSQL, con);
        OracleDataReader reader;
        selcmd.Parameters.Add(":dob", Convert.ToDateTime(dob.Text));
        selcmd.Parameters.Add(":pf_name", fname.Text);
        selcmd.Parameters.Add(":pn_name", lstname.Text);
        try
        {
            con.Open();
            ...
            if (reader.HasRows)
            {
                fn.Text = reader["pf_name"].ToString();
                ln.Text = reader["pn_name"].ToString();
                dobp.Text = reader["dob"].ToString();
                pati_id = reader["patient_id"].ToString();

            }
            else
            {
                errorresult.Text = ("No patients found");
            }
    ...
    protected void ReserveAppointment(string selectedSlot, string Selecteddoct, string selectedroom, DateTime startdate, DateTime enddate)
    {
        ...
        cmd.Parameters.Add(":patient_id", householdid);
